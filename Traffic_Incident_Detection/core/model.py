...
from backbones_2d.DeepLabV3PlusPytorch.network.modeling import _load_model
...
class YOWO(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        elif cfg.MODEL.BACKBONE_2D == "resnet101":
            self.backbone_2d = _load_model('deeplabv3plus', 'resnet101', num_classes=19,
                                           output_stride=9, pretrained_backbone=False)
            self.backbone_2d.load_state_dict(torch.load(cfg.WEIGHTS.BACKBONE_2D)['model_state'])
            self.backbone_2d = self.backbone_2d.backbone
            num_ch_2d = 2048
            self.custom2d_flag = True
        elif cfg.MODEL.BACKBONE_2D == "mobilenetv2":
            self.backbone_2d = _load_model('deeplabv3plus', 'mobilenetv2', num_classes=19,
                                           output_stride=16, pretrained_backbone=False)
            self.backbone_2d.load_state_dict(torch.load(cfg.WEIGHTS.BACKBONE_2D)['model_state'])
            self.backbone_2d = self.backbone_2d.backbone
            num_ch_2d = 320
            self.custom2d_flag = True
        elif cfg.MODEL.BACKBONE_2D == "other":
            pass
            # need right num channels
        else:
        ...
        if cfg.SOLVER.COMBINED_BOX:
            out_size = cfg.SOLVER.NUM_ANCHORS*(cfg.MODEL.NUM_CLASSES+4+1)
            self.conv_final = nn.Conv2d(1024, out_size, kernel_size=1,
                                        bias=False)
        else:
            out_size = cfg.SOLVER.NUM_ANCHORS * (cfg.LISTDATA.MAX_OBJS + 4 + 1)
            self.conv_final = nn.Conv2d(1024, out_size, kernel_size=1,
                                        bias=False)

        self.global_pool = nn.AvgPool2d(7,7)
        self.num_anomalies = cfg.MODEL.NUM_CLASSES
        self.linear_anomaly = nn.Linear(out_size, self.num_anomalies+1)
        # self.linear_anomaly = nn.Linear(out_size, self.num_anomalies)
    ...
    def forward(self, input):
        ...
        if self.custom2d_flag:
            x_2d = x_2d[next(reversed(x_2d))]
            x_2d = F.interpolate(x_2d, size=[7, 7],
                                 mode='bilinear', align_corners=False)

        x = torch.cat((x_3d, x_2d), dim=1)
        x = self.cfam(x)

        object_out = self.conv_final(x) # shape = b x 80 x 7 x 7
        x = self.global_pool(object_out) # shape = b x 80 x 1 x 1
        x = x.view(-1, x.shape[1]) # shape = b x 80
        anomaly_pred = self.linear_anomaly(x) # b x num_anom


        return object_out, anomaly_pred
